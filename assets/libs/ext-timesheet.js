(function () {
    ...
    Timesheet.prototype.insertData = function () {
        ...
        for (var n = 0, m = this.data.length; n < m; n++) {
            ...
            let featureLine = '';
            for (let p = 0, q = cur.features.length; p < q; p++) {
                featureLine = featureLine + '<span class="feature feature-' + cur.features[p].replace(/\s/g,  '').toLowerCase() + '">' + cur.features[p] + '</span>';
            }
            var line = [
    ...
    Timesheet.prototype.parse = function (data) {
        for (var n = 0, m = data.length; n < m; n++) {
            ...
            this.data.push({ start: beg, end: end, label: lbl, type: cat, features: feat });
        }
    };
